...
PAGES = {
    ("install", "npm"): {
        "title": "Install CodeWhale with npm (DeepSeek TUI Rename Guide)",
        "description": "Install CodeWhale with npm if you searched for DeepSeek TUI, verify the active binary, and avoid mixing old and new command names.",
        "eyebrow": "npm Install",
        ...
        "zh_title": "用 npm 安装 CodeWhale（DeepSeek TUI 改名指南）",
        "zh_description": "如果你是搜 DeepSeek TUI 进来的，这页会用当前 CodeWhale 包名解释 npm 安装、二进制验证和新旧命令切换。",
        "zh_eyebrow": "npm 安装",
        "zh_h1": "如果你是搜 DeepSeek TUI 进来的，现在该用 npm 安装的是 CodeWhale",
        "zh_intro": "项目上游已经改名，但真正的安装判断还是看你的 Node 工作流。如果你平时就通过 Node 管理终端工具，npm 路线通常最省事，只是现在应优先使用 CodeWhale 这套新包名和新命令。",
    ...
    ("install", "cargo"): {
        "title": "Install CodeWhale with cargo (DeepSeek TUI Rename Guide)",
        "description": "Use the cargo path for CodeWhale if you searched for DeepSeek TUI and your terminal workflow already lives in Rust tooling.",
        ...
        "zh_title": "用 cargo 安装 CodeWhale（DeepSeek TUI 改名指南）",
        "zh_description": "如果你是搜 DeepSeek TUI 进来的，而你的终端工具链本来就以 Rust 为主，这页会解释当前 CodeWhale 的 cargo 安装路线。",
        "zh_eyebrow": "Cargo 安装",
        "zh_h1": "如果 Rust 已经是你平时的终端工具链，现在更自然的路线是用 cargo 安装 CodeWhale",
        "zh_intro": "如果你是搜 DeepSeek TUI 进来的，先记住当前上游包名已经切到 CodeWhale。cargo 路线最适合本来就长期使用 Rust CLI、也习惯通过 cargo 更新工具的人。",
        "zh_answer_kicker": "最快判断",
    ...
    ("install", "homebrew"): {
        "title": "Install CodeWhale with Homebrew (DeepSeek TUI Rename Guide)",
        "description": "Use Homebrew to install CodeWhale if you searched for DeepSeek TUI and your macOS CLI workflow already relies on brew-managed tools.",
        "eyebrow": "Homebrew Install",
        "h1": "Homebrew is the cleanest CodeWhale install path when brew already owns most of your macOS CLI tools",
        "intro": "If you searched for DeepSeek TUI on macOS, the branding changed but the package-owner question did not. If your machine already treats Homebrew as the package authority for developer tools, using brew keeps updates and binary ownership clearer than introducing another package path just for one app.",
        "answer_kicker": "Best Use",
        ...
        "zh_title": "用 Homebrew 安装 CodeWhale（DeepSeek TUI 改名指南）",
        "zh_description": "如果你是搜 DeepSeek TUI 进来的，而你的 macOS CLI 本来就大量依赖 Homebrew，这页会解释当前 CodeWhale 的 brew 路线。",
        "zh_eyebrow": "Homebrew 安装",
        "zh_h1": "如果你的 macOS CLI 工具大多由 brew 管理，Homebrew 会是更整洁的 CodeWhale 安装路线",
        "zh_intro": "如果你是搜 DeepSeek TUI 进来的，先记住当前上游已经改名为 CodeWhale。当 Homebrew 已经是你机器上的主要包管理层时，用它安装会让更新路径和二进制归属更清晰。",
        "zh_answer_kicker": "最快判断",
    ...
    ("install", "windows"): {
        "title": "Install CodeWhale on Windows (DeepSeek TUI Rename Guide)",
        "description": "Choose the right Windows install path for CodeWhale if you searched for DeepSeek TUI and need the current package route for PowerShell, Git Bash, or WSL.",
        "eyebrow": "Windows Install",
        "h1": "Install CodeWhale on Windows by matching the package path to the shell you actually use every day",
        "intro": "If you searched for DeepSeek TUI on Windows, the current upstream name is CodeWhale. Windows is where install confusion grows fastest because the shell, PATH rules, and terminal habits vary much more than on macOS or Linux.",
        ...
        "zh_title": "在 Windows 上安装 CodeWhale（DeepSeek TUI 改名指南）",
        "zh_description": "如果你是搜 DeepSeek TUI 进来的，这页会按当前 CodeWhale 名字解释 PowerShell、Git Bash 和 WSL 的 Windows 安装路径。",
        "zh_eyebrow": "Windows 安装",
        "zh_h1": "在 Windows 上安装 CodeWhale，先看你每天真正工作的 shell，再决定安装路径",
        "zh_intro": "如果你是搜 DeepSeek TUI 进来的，先记住当前项目名已经切到 CodeWhale。Windows 安装最容易混乱，不是因为命令难，而是因为 shell、PATH 规则和终端习惯比 macOS、Linux 更分裂。",
    ...
    ("install", "update-or-upgrade"): {
        "title": "Update CodeWhale After the DeepSeek TUI Rename",
        "description": "Update or upgrade CodeWhale safely after the DeepSeek TUI rename by using the same package path that owns the active binary in your shell.",
        "eyebrow": "Update and Upgrade",
        "h1": "Updating CodeWhale is simple only after you confirm which package path actually owns the live binary",
        "intro": "Most upgrade confusion comes from duplicate installs, rename leftovers, or forgotten PATH history. The right update command is not universal; it depends on the package ecosystem that currently owns the active command your shell resolves first.",
        "answer_kicker": "Direct Answer",
        ...
        "zh_title": "更新或升级 CodeWhale（DeepSeek TUI 改名后）",
        "zh_description": "在 DeepSeek TUI 改名为 CodeWhale 之后，按当前真正拥有活跃二进制的包路径安全更新。",
        "zh_eyebrow": "更新与升级",
        "zh_h1": "只有先搞清楚当前活跃二进制到底归谁管，更新 CodeWhale 才会简单",
        "zh_intro": "DeepSeek TUI 改名以后，大多数升级混乱都来自重复安装、旧命令残留和 PATH 遗留。正确的升级命令不是放之四海而皆准的，它取决于现在 shell 第一个解析到的活跃命令到底来自哪条包管理路径。",
        "zh_answer_kicker": "直接答案",
    ...
    ("config", "api-key"): {
        "title": "Set Up the CodeWhale API Key (DeepSeek TUI Guide)",
        "description": "Configure the CodeWhale API key correctly if you searched for DeepSeek TUI, and separate provider auth problems from unrelated install issues.",
        "eyebrow": "API Key Setup",
        "h1": "API key setup is where many CodeWhale sessions succeed or fail before any mode or tool detail even matters",
        ...
        "zh_title": "设置 CodeWhale 的 API Key（DeepSeek TUI 指南）",
        "zh_description": "如果你是搜 DeepSeek TUI 进来的，这页会按当前 CodeWhale 路线解释 API Key 配置和 provider 认证问题。",
        "zh_eyebrow": "API Key 设置",
        "zh_h1": "在模式、工具和高级功能之前，API Key 设置往往就是决定 CodeWhale 能不能跑通的第一道关",
    ...
    ("config", "provider-setup"): {
        "title": "CodeWhale Provider Setup (DeepSeek TUI Guide)",
        "description": "Set up the right provider for CodeWhale if you searched for DeepSeek TUI, align auth with the selected backend, and avoid cross-provider config confusion.",
        "eyebrow": "Provider Setup",
        "h1": "Provider setup is where CodeWhale stops being a binary on disk and starts becoming a real working agent",
        ...
        "zh_title": "CodeWhale 的 Provider 设置（DeepSeek TUI 指南）",
        "zh_description": "如果你是搜 DeepSeek TUI 进来的，这页会按当前 CodeWhale 路线解释 provider 设置、认证和后端对齐。",
        "zh_eyebrow": "Provider 设置",
        "zh_h1": "provider 设置是 CodeWhale 从“磁盘上的命令”变成“真正可工作的代理”的关键一步",
    ...
    ("mcp", "setup"): {
        "title": "Set Up MCP for CodeWhale (DeepSeek TUI Guide)",
        ...
        "zh_title": "给 CodeWhale 设置 MCP（DeepSeek TUI 指南）",
        "zh_description": "如果你是搜 DeepSeek TUI 进来的，这页会按当前 CodeWhale 路线解释 MCP 设置顺序。",
        "zh_eyebrow": "MCP 设置",
        "zh_h1": "只有把 MCP 当成建立在稳定基础之上的高级层，CodeWhale 的 MCP 设置才会变清楚",
    ...
    ("mcp", "servers"): {
        "title": "CodeWhale MCP Servers (DeepSeek TUI Guide)",
        "description": "Understand CodeWhale MCP servers as workflow capability layers if you searched for DeepSeek TUI, not just as config names to copy.",
        "eyebrow": "MCP Servers",
        "h1": "CodeWhale MCP servers are most useful when you map each server to the exact workflow gap they fill",
        ...
        "zh_title": "CodeWhale 的 MCP Servers（DeepSeek TUI 指南）",
        "zh_description": "如果你是搜 DeepSeek TUI 进来的，这页会把当前 CodeWhale 的 MCP servers 解释成工作流能力层。",
        "zh_eyebrow": "MCP Servers",
        "zh_h1": "只有把每个 MCP server 映射回真实工作流缺口，CodeWhale 的 MCP servers 才算真正有用",
    ...
    ("mcp", "server-examples"): {
        "title": "CodeWhale MCP Server Examples (DeepSeek TUI Guide)",
        "description": "Use CodeWhale MCP server examples to understand which workflows each server extends before you copy config, if you searched for DeepSeek TUI.",
        ...
        "zh_title": "CodeWhale 的 MCP Server Examples（DeepSeek TUI 指南）",
        "zh_description": "如果你是搜 DeepSeek TUI 进来的，这页会用当前 CodeWhale 名字解释 MCP server examples 和工作流价值。",
